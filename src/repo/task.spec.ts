...
describe('save()', () => {
    ...
    it('MongoDBの状態が正常であれば、保管できるはず', async () => {
        ...
        const repository = new mocoin.repository.Task(mocoin.mongoose.connection);
...
describe('executeOneByName()', () => {
    ...
    it('MongoDBの状態が正常であれば、オブジェクトが返却されるはず', async () => {
        const taskName = mocoin.factory.taskName.MoneyTransfer;
...
describe('pushExecutionResultById()', () => {
    ...
    it('MongoDBの状態が正常であれば、オブジェクトが返却されるはず', async () => {
        const taskId = 'taskId';
        const status = mocoin.factory.taskStatus.Executed;
