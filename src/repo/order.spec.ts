...
describe('findByOrderInquiryKey()', () => {
    ...
    it('注文が存在すれば、取得できるはず', async () => {
        ...
        const repository = new mocoin.repository.Order(mocoin.mongoose.connection);
    ...
    it('存在しなければ、NotFoundエラーとなるはず', async () => {
        ...
        assert(result instanceof mocoin.factory.errors.NotFound);
        sandbox.verify();
    });
});
...
describe('changeStatus()', () => {
    ...
    it('注文が存在すればステータス変更できるはず', async () => {
        ...
        const orderStatus = mocoin.factory.orderStatus.OrderDelivered;
...
describe('注文を検索する', () => {
    ...
    it('MongoDBが正常であれば配列を取得できるはず', async () => {
        const orderRepo = new mocoin.repository.Order(mocoin.mongoose.connection);
        ...
        const result = await orderRepo.search({
            ...
            orderStatus: mocoin.factory.orderStatus.OrderCancelled,
            orderStatuses: [mocoin.factory.orderStatus.OrderCancelled],
