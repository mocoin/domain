...
describe('report2developers()', () => {
    ...
    it('LINE Notifyのアクセストークンを環境変数に未設定であれば、エラーになるはず', async () => {
        ...
        const result = await mocoin.service.notification.report2developers('', '', imageThumbnail, imageFullsize)()
            .catch((err) => err);
    ...
    it('LINE Notifyの200を返さなければ、エラーになるはず', async () => {
        ...
        const result = await mocoin.service.notification.report2developers('', '')()
            .catch((err) => err);
    ...
    it('imageThumbnailがURLでなければ、エラーになるはず', async () => {
        ...
        const result = await mocoin.service.notification.report2developers('', '', imageThumbnail)()
            .catch((err) => err);

        assert(result instanceof mocoin.factory.errors.Argument);
        assert(!scope.isDone());
    });
...
describe('sendEmailMessage()', () => {
    ...
    it('SendGridの状態が正常であればエラーにならないはず', async () => {
        const sendEamilMessageActionAttributets = {
            typeOf: mocoin.factory.actionType.SendAction,
            object: {
        ...
        const result = await mocoin.service.notification.sendEmailMessage(<any>sendEamilMessageActionAttributets)({ action: actionRepo });
