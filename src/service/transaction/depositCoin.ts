...
export function start(
    ...
    return async (repos: {
        ...
        const startParams: factory.transaction.IStartParams<factory.transactionType.DepositCoin> = {
            typeOf: factory.transactionType.DepositCoin,
            agent: params.agent,
        ...
        // 取引作成
        let transaction: factory.transaction.depositCoin.ITransaction;
        try {
            transaction = await repos.transaction.start(factory.transactionType.DepositCoin, startParams);
        } catch (error) {
...
function authorizeDepositCoinAccount(
    params: factory.transaction.depositCoin.ITransaction
...
export function confirm(params: {
    ...
    return async (repos: {
        ...
        const potentialActions: factory.transaction.depositCoin.IPotentialActions = {
            moneyTransfer: moneyTransferActionAttributes
        };

        // 取引確定
        debug('finally confirming transaction...');
        await repos.transaction.confirm(factory.transactionType.DepositCoin, transaction.id, transaction.object, {}, potentialActions);
    };
}
...
export function exportTasks(status: factory.transactionStatusType) {
    return async (repos: {
        ...
        const transaction = await repos.transaction.startExportTasks(factory.transactionType.DepositCoin, status);
        if (transaction === null) {
            return;
        }
...
export function exportTasksById(transactionId: string): ITaskAndTransactionOperation<factory.task.ITask[]> {
    return async (repos: {
        ...
        const transaction = await repos.transaction.findById(factory.transactionType.DepositCoin, transactionId);
