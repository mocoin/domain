...
export type ITransaction = factory.transaction.returnCoin.ITransaction;
...
export function start(
    params: factory.transaction.IStartParams<factory.transactionType.ReturnCoin>
    ...
    return async (repos: {
        ...
        // 取引オブジェクトを作成
        const startParams: factory.transaction.IStartParams<factory.transactionType.ReturnCoin> = {
            typeOf: factory.transactionType.ReturnCoin,
            agent: params.agent,
        ...
        // 取引作成
        let transaction: factory.transaction.returnCoin.ITransaction;
        try {
            transaction = await repos.transaction.start(factory.transactionType.ReturnCoin, startParams);
        } catch (error) {
        ...
        // 入金確認
        await authorizeDepositBankAccount(transaction)(repos);
...
function authorizeWithdrawCoinAccount(
    params: factory.transaction.returnCoin.ITransaction
    ...
    return async (repos: {
        ...
        if (params.object.fromLocation.typeOf !== factory.ownershipInfo.AccountGoodType.Account) {
            throw new factory.errors.Argument('params', 'params.object.fromLocation.typeOf must be Account');
        }
...
function authorizeDepositBankAccount(
    params: factory.transaction.returnCoin.ITransaction
    ...
    return async (repos: {
        ...
        if (params.object.toLocation.typeOf !== 'PaymentMethod') {
            throw new factory.errors.Argument('params', 'params.object.toLocation.typeOf must be PaymentMethod');
        }

        // 承認アクションを開始する
        const actionAttributes: factory.action.authorize.deposit.paymentMethod.bankAccount.IAttributes = {
            typeOf: factory.actionType.AuthorizeAction,
            object: {
        ...
        // アクションを完了
        debug('ending authorize action...');
        const actionResult: factory.action.authorize.deposit.paymentMethod.bankAccount.IResult = {
            amount: params.object.amount,
            pecorinoTransaction: pecorinoTransaction,
...
export function confirm(params: {
    ...
    return async (repos: {
        ...
        transaction = await repos.transaction.findById(factory.transactionType.ReturnCoin, transaction.id);
        ...
        const potentialActions: factory.transaction.returnCoin.IPotentialActions = {
            moneyTransfer: moneyTransferActionAttributes
        };

        // 取引確定
        debug('finally confirming transaction...');
        await repos.transaction.confirm(factory.transactionType.ReturnCoin, transaction.id, transaction.object, {}, potentialActions);
    };
}
...
export function exportTasks(status: factory.transactionStatusType) {
    return async (repos: {
        ...
        const transaction = await repos.transaction.startExportTasks(factory.transactionType.ReturnCoin, status);
        if (transaction === null) {
            return;
        }
...
export function exportTasksById(transactionId: string): ITaskAndTransactionOperation<factory.task.ITask[]> {
    return async (repos: {
        ...
        const transaction = await repos.transaction.findById(factory.transactionType.ReturnCoin, transactionId);
